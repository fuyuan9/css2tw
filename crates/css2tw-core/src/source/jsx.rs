            ...
            if ident.name.as_str() == "className" || ident.name.as_str() == "class" {
                if let Some(JSXAttributeValue::StringLiteral(lit)) = &attr.value {
                    ...
                    for part in class_names {
                        ...
                        let part_start = lit.value.as_str()[current_pos - start..].find(part).unwrap_or(0) + current_pos;
                    ...
                    if let JSXAttributeName::Identifier(ident) = &attr.name {
                        if ident.name.as_str() == "className" || ident.name.as_str() == "class" {
                            if let Some(JSXAttributeValue::StringLiteral(lit)) = &attr.value {
                                ...
                                let element_html = format!("<{} class=\"{}\"></{}>", tag_name, class_string, tag_name);
                                let fragment = Html::parse_fragment(&element_html);
                                if let Some(el) = fragment.root_element().select(&Selector::parse(tag_name).unwrap()).next() {
...
impl JsxParser {
    pub fn plan_jsx(
        ...
        let source_type = SourceType::from_path(&source.path).unwrap_or_default().with_jsx(true);
...
impl ClassUsageParser for JsxParser {
    fn extract_classes(&self, source: &SourceFile) -> Result<Vec<ClassUsage>, Css2TwError> {
        ...
        let mut visitor = ClassNameVisitor { classes: Vec::new() };
        visitor.visit_program(&ret.program);
