...
fn test_cli_stdin_support() {
    ...
    // Debug print
    if !json["changes"][0]["patched_content"].as_str().map_or(false, |p| p.contains("p-2.5")) {
        println!("JSON Output: {}", serde_json::to_string_pretty(&json).unwrap());
    }

    // Check if patched_content contains the tailwind class
    let patched = json["changes"][0]["patched_content"].as_str().expect("Missing patched_content");
    assert!(patched.contains("p-2.5") || patched.contains("p-[10px]"), "Expected p-2.5 or p-[10px] in patched content, found: {}", patched);
...
fn test_cli_raw_css_and_suggestions() {
    ...
    assert!(rep["raw_css"].as_str().is_some(), "Expected raw_css to be populated");
